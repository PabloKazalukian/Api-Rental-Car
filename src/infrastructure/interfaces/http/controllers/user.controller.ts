...
export class UserController {
  ...
  async getUserById(req: Request, res: Response): Promise<Response> {
    try {
      const data: UserEntity | null = await this.userService.findById(
        req.params.idUser
      );
      if (!data)
  ...
  async createUser(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (userExist)
        return this.httpResponse.Error(res, "El email ya está en uso");

      const userExistUsername = await this.userService.findUserByUsername(
        req.body.username
      );
