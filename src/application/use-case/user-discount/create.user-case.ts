...
export class CreateUserDiscountUseCase {
    ...
    async execute(uDiscount: CreateUserDiscountDTO): Promise<UserDiscount> {
        ...
        if (user === null) { throw new HttpException(HttpStatus.UNAUTHORIZED, UserErrorMessages.USER_NOT_FOUND); }

        const discount = await this.discountSvc.findById(uDiscount.discountId);
        if (discount === null) { throw new HttpException(HttpStatus.UNAUTHORIZED, DiscountErrorMessages.DISCOUNT_NOT_FOUND); }
